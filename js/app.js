...
app_angular.controller('sessionController',['bootbox','Conexion','$scope','$location','$http','$route', '$routeParams', 'Factory','actividadesFactory' ,function (bootbox,Conexion, $scope, $location, $http,$route, $routeParams, Factory,actividadesFactory) {
    ...
    $scope.alerta=[];
    $scope.errorAlerta=[];
    $scope.$watch('online', function(newStatus,oldValue) 
        {
    ...
    $scope.uploadData=function()
    {
        ...
        //ENVIO DE ACTIVIDADES CON LOCALIZACION
        Mensajes('Sincronizacion iniciada','information','')
        CRUD.selectAllinOne("select a.*,l.rowid rowidl,l.rowid_actividad,l.latitud,l.longitud,l.descripcion descripcionl,l.estadoID estadoIDl from crm_localizacion l inner join crm_actividades a on  a.rowid=l.rowid_actividad order by l.rowid",function(Listado){
            ...
            for (var i = 0; i < Listado.length; i++) {
                if ($scope.status.connextionstate==false) {
                 ...
                 $.ajax({
                    url:'http://demoedex.pedidosonline.co/Mobile/uploadData',
                    async:false,
                    data:{
                        usuario:$scope.usuario,
                        entidad:"ACTIVIDADES",
                        codigo_empresa:$scope.codigoempresa,
                        datos:JSON.stringify(Listado[i])
                    },
                    success:function(data)
                    {
                        CRUD.Updatedynamic("delete from crm_localizacion where  rowid="+data.rowid+"");
                    },
                    error:function(data)
                    {
                        $scope.errorAlerta.bandera=1;
        ...
        CRUD.selectAllinOne("select*from s_planos_pedidos where estado=0 order by ultimo_registro asc ",function(Listado){
            for (var i = 0; i < Listado.length; i++) {
                if ($scope.status.connextionstate==false) {
                    $scope.errorAlerta.bandera=1;
                    continue;
                ...
                $.ajax({
                    url:"http://demoedex.pedidosonline.co/Mobile/syncEdex",
                    async:false,
                    data:
                    {
                        usuario:$scope.usuario,
                    ...
                    success:function(data)
                    {
                        CRUD.Updatedynamic("update s_planos_pedidos set estado=1 where rowid="+data.rowid+"");
                    },
                    error:function(data)
                    {
                        $scope.errorAlerta.bandera=1;
        ...
        {
            Mensajes('Sincronizacion Culmino con errores por favor intente de nuevamente','error','');
            ProcesadoHiden();   
        }
    }
    ...
    $scope.downloadData=function(usuario,empresa)
    {
        for(var i=0; i < STEP_SINCRONIZACION.length; i++)
        {
            if ($scope.status.connextionstate==false) 
            ...
            $.ajax({
                ...
                type: 'get',
                url: 'http://demoedex.pedidosonline.co/Mobile/Instalacion?step=2&username='+usuario+'&codigo_empresa=' + empresa+ '&entidad=' + STEP_SINCRONIZACION[i],
                async: false,
                timeout:'12000',
                success: function (data) {
                        DATOS_ENTIDADES_SINCRONIZACION[i]=STEP_SINCRONIZACION[i].toString();
                        DATOS_ENTIDADES_SINCRONIZACION[i] = data;
                },
                error:function(data)
                {
                    $scope.errorAlerta.bandera=1;
    ...
    $scope.sincronizar=function(){
        ...
        window.setTimeout(function(){
            ...
            $scope.downloadData($scope.sessiondate.nombre_usuario,$scope.sessiondate.codigo_empresa);
            if ($scope.errorAlerta.bandera==1) 
            {
                Mensajes('Por Favor Revisar conexion Estable','error','')
                ProcesadoHiden();
                return
            }
            var contador=0;
